...
void read_all_motor_data(motor_t *motor)
{
    ...
    {
		case 1:  read_motor_data(motor->hcan,motor->id, RID_UV_VALUE);  break; // UV_Value
		case 2:  read_motor_data(motor->hcan,motor->id, RID_KT_VALUE);  break; // KT_Value
		case 3:  read_motor_data(motor->hcan,motor->id, RID_OT_VALUE);  break; // OT_Value
		case 4:  read_motor_data(motor->hcan,motor->id, RID_OC_VALUE);  break; // OC_Value
		case 5:  read_motor_data(motor->hcan,motor->id, RID_ACC);       break; // ACC
		case 6:  read_motor_data(motor->hcan,motor->id, RID_DEC);       break; // DEC
		case 7:  read_motor_data(motor->hcan,motor->id, RID_MAX_SPD);   break; // MAX_SPD
		case 8:  read_motor_data(motor->hcan,motor->id, RID_MST_ID);    break; // MST_ID 
		case 9:  read_motor_data(motor->hcan,motor->id, RID_ESC_ID);    break; // ESC_ID
		case 10: read_motor_data(motor->hcan,motor->id, RID_TIMEOUT);   break; // TIMEOUT 
		case 11: read_motor_data(motor->hcan,motor->id, RID_CMODE);     break; // CTRL_MODE 
		case 12: read_motor_data(motor->hcan,motor->id, RID_DAMP);      break; // Damp 
		case 13: read_motor_data(motor->hcan,motor->id, RID_INERTIA);   break; // Inertia
		case 14: read_motor_data(motor->hcan,motor->id, RID_HW_VER);    break; // Rsv1 
		case 15: read_motor_data(motor->hcan,motor->id, RID_SW_VER);    break; // sw_ver 
		case 16: read_motor_data(motor->hcan,motor->id, RID_SN);        break; // Rsv2 
		case 17: read_motor_data(motor->hcan,motor->id, RID_NPP);       break; // NPP 
		case 18: read_motor_data(motor->hcan,motor->id, RID_RS);        break; // Rs 
		case 19: read_motor_data(motor->hcan,motor->id, RID_LS);        break; // Ls 
		case 20: read_motor_data(motor->hcan,motor->id, RID_FLUX);      break; // Flux 
		case 21: read_motor_data(motor->hcan,motor->id, RID_GR);        break; // Gr 
		case 22: read_motor_data(motor->hcan,motor->id, RID_PMAX);      break; // PMAX 
		case 23: read_motor_data(motor->hcan,motor->id, RID_VMAX);      break; // VMAX 
		case 24: read_motor_data(motor->hcan,motor->id, RID_TMAX);      break; // TMAX 
		case 25: read_motor_data(motor->hcan,motor->id, RID_I_BW);      break; // I_BW 
		case 26: read_motor_data(motor->hcan,motor->id, RID_KP_ASR);    break; // KP_ASR 
		case 27: read_motor_data(motor->hcan,motor->id, RID_KI_ASR);    break; // KI_ASR 
		case 28: read_motor_data(motor->hcan,motor->id, RID_KP_APR);    break; // KP_APR 
		case 29: read_motor_data(motor->hcan,motor->id, RID_KI_APR);    break; // KI_APR 
		case 30: read_motor_data(motor->hcan,motor->id, RID_OV_VALUE);  break; // OV_Value 
		case 31: read_motor_data(motor->hcan,motor->id, RID_GREF);      break; // GREF 
		case 32: read_motor_data(motor->hcan,motor->id, RID_DETA);      break; // Deta 
		case 33: read_motor_data(motor->hcan,motor->id, RID_V_BW);      break; // V_BW 
		case 34: read_motor_data(motor->hcan,motor->id, RID_IQ_CL);     break; // IQ_c1 
		case 35: read_motor_data(motor->hcan,motor->id, RID_VL_CL);     break; // VL_c1 
		case 36: read_motor_data(motor->hcan,motor->id, RID_CAN_BR);    break; // can_br 
		case 37: read_motor_data(motor->hcan,motor->id, RID_SUB_VER);   break; // sub_ver 
		case 38: read_motor_data(motor->hcan,motor->id, RID_U_OFF);     break; // u_off 
		case 39: read_motor_data(motor->hcan,motor->id, RID_V_OFF);     break; // v_off 
		case 40: read_motor_data(motor->hcan,motor->id, RID_K1);        break; // k1 
		case 41: read_motor_data(motor->hcan,motor->id, RID_K2);        break; // k2 
		case 42: read_motor_data(motor->hcan,motor->id, RID_M_OFF);     break; // m_off 
		case 43: read_motor_data(motor->hcan,motor->id, RID_DIR);       break; // dir 
		case 44: read_motor_data(motor->hcan,motor->id, RID_P_M);       break; // pm 
		case 45: read_motor_data(motor->hcan,motor->id, RID_X_OUT);     break; // xout 
    }
}
/**
************************************************************************
