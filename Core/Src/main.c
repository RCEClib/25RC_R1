...
// 底盘控制全局结构体定义
// 作用：存储底盘所有控制参数、PID控制器、电机状态等信息
// 设计选择：使用全局变量而非局部变量的原因：
// 1. 生命周期需求：底盘控制需要在整个程序运行期间持续存在
// 2. 性能考虑：避免频繁的栈内存分配和释放
...
int main(void)
{
  ...
  while (1)
  {
    ...
    scissor_lift_Task(remoter.var.S1/5,remoter.var.S2*0.0628,remoter.key.SB,remoter.key.SC,remoter.key.SD);
